...
def validate_aadhaar(text):
    ...
    # Look for all 12-digit patterns with spaces (4-4-4 format)
    # Using findall and taking the last match specifically avoids mobile numbers/noise
    aadhaar_pattern = r'\b\d{4}\s\d{4}\s\d{4}\b'
    aadhaar_matches = re.findall(aadhaar_pattern, raw_text)
    
    # 2. FIXED DATE OF BIRTH LOGIC
    # Handles both 'DD/MM/YYYY' and common OCR errors like 'aadOB: 03041981'
    dob_match = re.search(r'(\d{2}/\d{2}/\d{4})|(\d{8})', raw_text)
    if dob_match:
        dob = dob_match.group(0)
        # If it found 8 digits without slashes, format it
    ...
    # 3. FIXED NAME LOGIC
    # Improved pattern to catch 'Bajit Shalk' near the ID or 'To' headers
    name_pattern = r'(?:To|08515)\s+([A-Z][a-z]+\s[A-Z][a-z]+)'
